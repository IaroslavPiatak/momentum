...
const url = `http://www.7timer.info/bin/api.pl?lon=${localStorage.getItem('longitude')}&lat=${localStorage.getItem('latitude')}&product=civil&output=json`;
...
fetch(url)
    ...
    .then(data => {
      ...
      localStorage.setItem('weatherCondition', weatherConditionsObj[weatherCondition]);
...
document.querySelector('.weather').textContent = `${localStorage.getItem('city')} | ${localStorage.getItem('temperature')}°C | ${localStorage.getItem('weatherCondition')}`